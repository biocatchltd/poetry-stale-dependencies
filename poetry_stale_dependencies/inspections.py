    ...
    def inspect(self, session: Client, com: Command)->bool:
        ...
        for local_version in self.versions:
            ...
            if (local_spec := remote.by_version.get(local_version)) is None:
                com.line_error(f"Local version {self.package} {local_version} not found in remote, skipping", verbosity=Verbosity.NORMAL)
                continue
            ...
            if latest_time > stale_time:
                ...
                com.line(f"local version {local_version} of {self.package} is stale, latest is {latest.version} (delta: {render_timedelta(delta)})", verbosity=Verbosity.NORMAL)
                com.line(f"\t{local_version} was uploaded at {local_version_time.isoformat()}, {latest.version} was uploaded at {latest_time.isoformat()}", verbosity=Verbosity.VERBOSE)
                ...
                if oldest_non_stale is not None:
                    com.line(f"\toldest non-stale release is {oldest_non_stale[0].version} ({oldest_non_stale[1].isoformat()})", verbosity=Verbosity.VERBOSE)
        return ret
